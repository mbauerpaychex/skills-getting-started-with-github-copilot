      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (participant) =>
                  `<li class="participant-item"><span class="participant-email">${participant}</span><button type="button" class="delete-participant-btn" data-activity="${name}" data-participant="${participant}" aria-label="Remove ${participant} from ${name}" title="Unregister participant">X</button></li>`
              )
              .join("")}</ul>`
